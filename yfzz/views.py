...
def doctorinsert(request):
    ...
    if request.session.get('opid', '')=='':#没有id
        ...
        if request.method == 'POST':
            ...
            del request.session['opid']
            return render(request, 'infopage.html', {'errorinfo': '成功添加医生账户！', 'goback': '/adminhomepage'})
        else:
            del request.session['opid']
...
def doctordelete(request):
    ...
    if request.session.get('opid', '')=='':#没有id
        if request.method == 'POST' and request.POST['id']!='':
            ...
            try:
                ...
                return render(request, 'infopage.html',
                              {'errorinfo': '此医师不属于' + hh.name + '！', 'goback': '/adminhomepage'})
            else:
        ...
        if request.method == 'POST':
            ...
            del request.session['opid']
            return render(request, 'infopage.html', {'errorinfo': '成功删除医生账户！',
                                                     'goback': '/adminhomepage'})
        else:
            del request.session['opid']
        ...
        if m.type=='1':
            ...
            try:
                ...
                hpt.Message.objects.using(m.tohid).create(fromhid=m.fromhid,
                                                          ...
                                                          extra=m.extra)
                del request.session['oppid']
                return render(request, 'infopage.html', {'errorinfo': u'您已成功接收该病人！',
                                                         'goback': '/doctorlogin'})
    else:
        try:
            pid=request.GET['pid']
...
def change(request):
    ...
    p=hpt.Patient.objects.using(request.session['p_h']).get(pid=request.session['p_id'])
    if p.is_changing:
        return render(request,'infopage.html',{'errorinfo':'您的转诊请求已经通知主治医师，请耐心等待！',
